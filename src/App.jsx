...
function Button({ children, className = '', variant = 'solid', ...props }) {
  const base = 'inline-flex items-center justify-center rounded-full font-black transition focus:outline-none focus:ring-4 focus:ring-rose-200'
  const variants = {
    solid: 'bg-rose-600 text-white hover:bg-rose-700',
    dark: 'bg-slate-950 text-white hover:bg-slate-800',
    light: 'bg-white text-slate-950 hover:bg-rose-50',
...
const faqs = [
  {
    q: 'KCL 같은 대형기관 대신 YSIET에 문의해야 하는 이유가 뭔가요?',
    ...
    <main className="min-h-screen bg-[#fff8f4] text-slate-950">
      <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
          ...
          <nav className="hidden items-center gap-7 text-sm font-bold text-slate-600 xl:flex">
            <a href="#compare" className="hover:text-slate-950">KCL 비교</a>
          ...
          <a href="#contact">
            <Button className="px-5 py-2.5">납기 확인</Button>
          </a>
        </div>
      </header>
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-5 py-16 lg:grid-cols-[1.07fr_0.93fr] lg:items-center lg:py-24">
          <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm font-black text-rose-100 shadow-sm backdrop-blur">
              ...
              KCL과 비교 중이라면, 먼저 성적서 납기부터 확인하세요
            </div>
            ...
            <p className="mt-7 max-w-2xl text-xl leading-9 text-slate-200">
              식약처 지정 제18호와 KOLAS 제364호의 신뢰성. 일반 7일·긴급 3일 가능 여부 상담. 자가품질 위탁검사부터 시험성적서 발급까지 바로 안내합니다.
            </p>
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              {proofBadges.map((item) => (
                <div key={item} className="flex items-center gap-2 rounded-2xl bg-white/10 px-4 py-3 text-sm font-black text-white ring-1 ring-white/15">
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              ...
              <a href="#compare">
                <Button variant="outline" className="h-14 px-8 text-base">왜 YSIET인가</Button>
              </a>
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.1 }}>
            <Card className="rounded-[2rem] border-0 bg-white text-slate-950 shadow-2xl shadow-rose-900/40 ring-0">
              <div className="p-7 md:p-9">
                <div className="mb-7 rounded-3xl bg-gradient-to-br from-rose-600 to-orange-500 p-6 text-white">
                  ...
                  <h2 className="mt-2 text-3xl font-black">KCL 검색하다 오셨나요?</h2>
                  <p className="mt-3 leading-7 text-rose-50">그럼 아래 4가지만 먼저 확인하세요. 납기, 지정기관, KOLAS, 수수료.</p>
                </div>
                ...
                <div className="mt-7 grid gap-3 sm:grid-cols-2">
                  <a href="tel:02-312-0540" className="flex items-center justify-center gap-2 rounded-2xl bg-rose-600 px-4 py-4 text-center font-black text-white transition hover:bg-rose-700">
                    <Phone className="h-5 w-5" /> 02-312-0540
                  </a>
                  <a href="mailto:testing@ysiet.com" className="flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-4 py-4 text-center font-black text-white transition hover:bg-slate-800">
      ...
      <section id="compare" className="mx-auto max-w-7xl px-5 py-24">
        <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.25em] text-rose-600">KCL Comparison</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight md:text-6xl">비교하다가도 YSIET에 문의하게 만드는 이유.</h2>
            <p className="mt-6 text-lg leading-8 text-slate-700">
              고객은 긴 기관 소개보다 “내 성적서가 언제 나오고, 얼마쯤 들고, 무엇을 준비해야 하는지”를 먼저 알고 싶어 합니다. 그래서 YSIET는 결정에 필요한 정보를 앞에 둡니다.
            </p>
            <a href="#contact" className="mt-8 inline-flex rounded-full bg-slate-950 px-7 py-4 font-black text-white transition hover:bg-slate-800">
              KCL과 비교 전 납기 확인하기
            </a>
          </div>
          <div className="overflow-hidden rounded-[2rem] bg-white shadow-xl ring-1 ring-slate-200">
            <div className="grid grid-cols-2 bg-slate-950 text-white">
              <div className="p-5 text-sm font-black text-slate-300">대형기관식 페이지에서 고객이 느끼는 점</div>
              <div className="bg-rose-600 p-5 text-sm font-black">YSIET 랜딩페이지가 바로 답하는 것</div>
            </div>
            {comparisonRows.map(([before, after]) => (
              <div key={before} className="grid grid-cols-2 border-t border-slate-100">
                <div className="p-5 font-bold text-slate-500 line-through decoration-rose-400/70">{before}</div>
      ...
      <section className="bg-slate-950 py-24 text-white">
        <div className="mx-auto max-w-7xl px-5">
          <div className="mb-12 max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[0.25em] text-rose-300">Decision Factors</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight md:text-6xl">고객의 결정 버튼은 이 4개에서 눌립니다.</h2>
          </div>
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {decidingFactors.map((item) => (
              <div key={item.title} className="rounded-3xl border border-white/10 bg-white/10 p-7 text-white shadow-xl">
      ...
      <section id="services" className="mx-auto max-w-7xl px-5 py-24">
        <div className="mb-12 grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.25em] text-rose-600">Testing Services</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">검사 가능한 서비스를 한눈에.</h2>
          </div>
          <p className="text-lg leading-8 text-slate-700">
            일반 화장품, 기능성 화장품, 원료·부자재, R&D 지원까지. 정확한 가능 여부와 세부 항목은 제품 유형과 제출 목적에 따라 상담 후 확정됩니다.
          </p>
        </div>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {services.map((service) => (
            <div key={service} className="flex items-start gap-4 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
      ...
      <section id="fees" className="bg-white py-24">
        <div className="mx-auto max-w-7xl px-5">
          <div className="mb-12 grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.25em] text-rose-600">Fee Guide</p>
              <h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">가격 감을 먼저 잡게 해드립니다.</h2>
            </div>
            <p className="text-lg leading-8 text-slate-700">
              고객이 문의를 미루는 가장 큰 이유 중 하나는 비용 불확실성입니다. 대표 수수료를 먼저 보여주고, 실제 견적은 시험항목·제품유형·긴급 여부에 따라 상담 후 안내합니다. 모든 금액은 VAT 별도입니다.
            </p>
          </div>
          ...
          <div className="mt-6 rounded-3xl bg-slate-950 p-6 text-white md:flex md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 text-slate-300">제품명과 필요 항목을 보내주시면 실제 견적 기준으로 안내드립니다.</p>
            </div>
            <a href="#contact" className="mt-5 inline-flex rounded-full bg-rose-600 px-6 py-3 font-black text-white md:mt-0">견적 요청하기</a>
          </div>
        </div>
      </section>
      ...
      <section id="self-quality" className="mx-auto max-w-7xl px-5 py-24">
        <div className="grid gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.25em] text-rose-600">Self Quality Testing</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">자가품질 위탁검사, 처음이어도 바로 시작할 수 있게.</h2>
            <p className="mt-6 text-lg leading-8 text-slate-700">
              화장품 제조업자·책임판매업자는 품질검사 시설이 없는 경우 식약처 지정 시험·검사기관과 품질검사 위탁계약을 체결하여 품질관리를 진행할 수 있습니다. YSIET는 계약, 시험의뢰, 시료 접수, 성적서 발급까지 한 번에 안내합니다.
            </p>
          </div>
      ...
      <section id="documents" className="bg-slate-950 py-24 text-white">
        <div className="mx-auto grid max-w-7xl gap-12 px-5 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.25em] text-rose-300">Before Request</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">문의 전 준비하면 견적이 빨라집니다.</h2>
      ...
      <section id="trust" className="py-24">
        <div className="mx-auto max-w-7xl px-5">
          <div className="mb-12 max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[0.25em] text-rose-600">Trust Proof</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">신뢰근거는 숨기지 않고, 반복해서 설득합니다.</h2>
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {[
              [ShieldCheck, 'MFDS', '식약처 지정 화장품 시험·검사기관', '제18호 지정기관으로 화장품 시험·검사와 자가품질 위탁검사 상담이 가능합니다.'],
              [Award, 'KOLAS', '국제공인시험기관', 'KOLAS 제364호 국제공인시험기관으로 분석 결과의 신뢰성을 강조할 수 있습니다.'],
              [Microscope, 'EXPERT', '석·박사 분석/연구 전담 인력', '숙련된 연구진과 분석 장비를 기반으로 신속·정확·정밀한 분석 결과를 제공합니다.'],
              [Users, 'COUNSELING', '1건 의뢰도 전문 카운셀링', '화장품 분석뿐 아니라 제품개발, 관련법령 준수, 준비서류까지 상담합니다.'],
      ...
      <section id="process" className="bg-white py-24">
        <div className="mx-auto max-w-7xl px-5">
          <div className="mb-12 max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[0.25em] text-rose-600">Process</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">복잡한 설명 없이, 성적서까지 4단계.</h2>
            <p className="mt-5 text-lg leading-8 text-slate-700">
      ...
      <section id="contact" className="mx-auto max-w-7xl px-5 py-24">
        <div className="overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-rose-600 to-orange-500 p-8 text-white shadow-2xl shadow-rose-200 md:p-12">
          <div className="grid gap-8 lg:grid-cols-[1fr_0.85fr] lg:items-center">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.25em] text-rose-100">Request Quote</p>
              <h2 className="mt-3 text-4xl font-black tracking-tight md:text-6xl">KCL 비교는 끝. 이제 YSIET에 납기부터 확인하세요.</h2>
              <p className="mt-6 max-w-2xl text-lg leading-8 text-rose-50">
                일반 7일 · 긴급 3일 가능 여부 상담. 제품명, 제형, 필요한 성적서 용도, 희망 납기를 알려주시면 우선 검토 후 안내드립니다.
              </p>
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <a href="tel:02-312-0540" className="inline-flex h-14 items-center justify-center gap-2 rounded-full bg-white px-8 font-black text-slate-950 transition hover:bg-rose-50">
                  <Phone className="h-5 w-5" /> 02-312-0540
                </a>
                <a href="mailto:testing@ysiet.com" className="inline-flex h-14 items-center justify-center gap-2 rounded-full bg-slate-950 px-8 font-black text-white transition hover:bg-slate-800">
            ...
            <div className="rounded-[2rem] bg-white p-6 text-slate-950 shadow-xl">
              ...
              <a href="mailto:testing@ysiet.com">
                <Button variant="dark" className="mt-6 h-14 w-full rounded-2xl text-base">빠른 견적 요청하기</Button>
              </a>
      ...
      <section id="faq" className="bg-[#fffaf7] py-24">
        <div className="mx-auto max-w-5xl px-5">
          <div className="mb-10 text-center">
            <p className="text-sm font-black uppercase tracking-[0.25em] text-rose-600">FAQ</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">망설이는 고객의 마지막 질문까지 답합니다.</h2>
          </div>
            ...
            {faqs.map((faq) => (
              <details key={faq.q} className="group rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-lg font-black">
                  <span className="flex items-center gap-3"><HelpCircle className="h-5 w-5 text-rose-600" />{faq.q}</span>
      ...
      <div className="fixed bottom-4 left-1/2 z-50 w-[calc(100%-2rem)] max-w-3xl -translate-x-1/2 rounded-full bg-slate-950 p-2 shadow-2xl md:hidden">
        <div className="grid grid-cols-2 gap-2">
          <a href="tel:02-312-0540" className="rounded-full bg-white px-4 py-3 text-center text-sm font-black text-slate-950">전화 상담</a>
          <a href="mailto:testing@ysiet.com" className="rounded-full bg-rose-600 px-4 py-3 text-center text-sm font-black text-white">이메일 견적</a>
        </div>
      </div>
    </main>
  )
